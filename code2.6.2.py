...
data = np.loadtxt('Facility.txt')

x_coords = data[:, 1]
y_coords = data[:, 2]
costs = data[:, 4]

grid_x, grid_y = np.meshgrid(np.linspace(1, 9, 100), np.linspace(0, 9, 100))


rbf = Rbf(x_coords, y_coords, costs, function='cubic')
...
plt.scatter(x_coords, y_coords, c='red', edgecolors='black', s=100)
...
RBF = model.addVars(k, lb = 0, vtype = GRB.CONTINUOUS)

w = [40.44825277, -88.82891205, 184.41470346 , -160.72320466, 40.14017977, -16.82948401, 48.15241884, -48.61122826, -160.13747366, 144.25268697]
   
for i in range(k) :
   model.addConstr(RBF[i] == gp.quicksum(w[j]*(((X_f[i] - x_coords[j])**2) + (Y_f[i] - y_coords[j])**2) for j in range(10)))
...
model.setObjective(gp.quicksum(M[i] for i in range(k)) + gp.quicksum(RBF[i] for i in range(k)), GRB.MINIMIZE)
...
for i in range(k) : Sum_Cost += RBF[i].x
